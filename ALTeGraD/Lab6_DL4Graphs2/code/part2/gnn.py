...
for epoch in range(epochs):
    ...
    for i in range(0, N_train, batch_size):
        ...
        ##################
        features_batch = list()
        for j in range(i, min(i+batch_size, len(y_train))):
            G = G_train[j]
            N_G = G.number_of_nodes()
            adj_batch.append(nx.adjacency_matrix(G) + sp.eye(N_G))
            idx_batch += (N_G*[j-i])
            y_batch.append(y_train[j])
            features_batch += (N_G*[1])
            
        adj_batch = sp.block_diag(adj_batch)
        adj_batch = sparse_mx_to_torch_sparse_tensor(adj_batch)
        idx_batch = torch.LongTensor(idx_batch)
        y_batch = torch.LongTensor(y_batch)
        features_batch = torch.FloatTensor(features_batch)[:,None]
        ##################
...
for i in range(0, N_test, batch_size):
    ...
    ##################
    features_batch = list()
    for j in range(i, min(i+batch_size, len(y_test))):
        G = G_test[j]
        N_G = G.number_of_nodes()
        adj_batch.append(nx.adjacency_matrix(G) + sp.eye(N_G))
        idx_batch += (N_G*[j-i])
